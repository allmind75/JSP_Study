		...
		<ul>
			<li class="notice"><i class="fa fa-chevron-down notice-icon"></i>
				...
				<div class="notice-context">
					<p class="context">
						진주투어는 진주시의 관광지, 맛집, 특산물 정보를<br /> 제공하는 여행서비스입니다.
					</p>
				</div></li>
			<li class="notice"><i class="fa fa-chevron-down notice-icon"></i>
				...
				<div class="notice-context">
					<p class="context">
						로그인 화면에서 아이디 찾기 또는 비밀번호 찾기를<br /> 터치한 후, 가입당시 입력하신 이메일을 입력하시면<br />
						아이디 또는 임시비밀번호를 보내드립니다.
					</p>
				</div></li>
		</ul>
	</section>
	...
	<script>
		$(".notice").click(
				function(e) {
					...
					if (state == "none") {
						$(this).find('div').show();
						$(this).find('i').removeClass("notice-icon").addClass(
								"notice-active");
					} else {
						$(this).find('div').hide();
						$(this).find('i').removeClass("notice-active")
